...
> 基于 mica-ppocr 1.1.3 源码（含 sources jar）逐行核对，默认值取自
> `PPOcrV6Config.defaults()` 实测输出。第二节的耗时基准已在 1.1.3 复测，
> 测试环境为 Apple Silicon（4 性能核 + 6 能效核），测试图为仓库自带的
> `test_images/1.png`（2988×2199 行驶证照片），预热后每档 3 次取中位数。
...
测试图为仓库自带的 `test_images/1.png`（2988×2199 行驶证照片），全默认配置，
每档跑 3 次取中位数。**这份数据可复现**——图在仓库里，跑 `scripts/quality-gate.sh`
或直接打 `/api/ocr` 即可复核。

| 档次 | 耗时（1.1.3） | 检出框数 | 耗时（1.0.1 旧记录） |
|------|--------------|---------|---------------------|
| tiny | ~1.36s | 36 | ~1.0s |
| small | ~3.68s | 37 | ~2.8s |
| medium | ~19.3s | 37 | ~14s |
...
1. **检出框数与 1.0.1 完全一致**（36 / 37 / 37）——升级 mica-ppocr 没有带来识别行为变化。
   这比耗时更重要：框数一致意味着下游的字段解析不受影响。
2. **耗时比 1.0.1 的记录高约 35%，原因未能完全确认**。已排除的一项：1.1.1+ 新增的
   `doc_ori` 方向分类只占 3~4%（tiny 关掉后 1.36s → 1.32s，small 3.68s → 3.53s）。
   剩余差异可能来自 ORT 版本降级（1.0.1 配 ORT 1.26.0，1.1.3 配 1.18.0），
   也可能是两次测量的机器状态不同——旧记录没有留下足够信息来对齐环境，无法定论。
3. **medium 档已逼近默认超时**。19.3s 对应的是 2988×2199；更大的图会直接撞上
   `ocr.timeout-ms: 30000`（实测 2284×3305 的护照图在 medium 档要 29.6s，首次请求
   叠加模型加载会超时报 2002）。用 medium 处理大图前务必调大超时或预先压缩。

降分辨率与多线程的效果（这两组测于 1.0.1，未复测，仅作趋势参考）：
